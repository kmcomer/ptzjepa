...
In the following figure we show the training loss of the world model and the reinforcement learning agent during the training process. The world model is trained with the images collected by the camera and the reinforcement learning agent is trained with the dreams generated by the world model (which are sequences of internal representations (embedding vectors) that are then used to train the reinforcement learning agent).

<img src="figures/Loss.png" alt="Lifelong training loss plots" width="600"/>
...
## Analyzing Jupyter Lab output figures:

The figure bellow shows a chart with a tSNE dimensionality reduction of the embedding vectors collected by the camera. The chart shows the embedding vectors in a 2D space. The colors of the points in the chart represent the different rewards received by the agent. The chart shows that the embedding vectors are somehow clustered in different regions of the 2D space in relation to the rewards received by the agent.

<img src="figures/tsne.png" alt="tSNE chart" width="600"/>

In the next figure below we show two plots with the PAN-TILT location of the camera colored by reward and ZOOM level. The plots show that the camera is moving to different locations in the environment and the zoom level is changing as well. The color of the points in the plot represents the reward received by the agent for the upper plot and the zoom level for the lower plot. 

<img src="figures/ptrz.png" alt="PTRZ chart" width="600"/>

It is clear that the agent presents diffrent behaviors and receives different rewards for different locations in the PTZ space.

The next figure below shows a 3D plot of the PAN-TILT-ZOOM location of the camera colored by reward. The plot shows that the camera is moving to different locations in the environment and the zoom level is changing as well. The color of the points in the plot represents the reward received by the agent.

<img src="figures/3dptrz.png" alt="3D PTRZ chart" width="600"/>

Finally the last couple of plots below show two episode examples of the camera moving in the environment for two different areas of the space and different reward levels.

<img src="figures/episode1.png" alt="Episode 1" width="600"/>

<img src="figures/episode2.png" alt="Episode 2" width="600"/>

We can see the trajectory of the camera in the environment and the reward received by the agent for each action taken by the camera. The color of the points in the plot represents the reward received by the agent.
The evolution of the reward movement after movement shos that the reward grows as the camera moves to different locations.
